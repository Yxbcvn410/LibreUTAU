                ...
                if (line.Trim().StartsWith(@"[#") && line.Trim().EndsWith(@"]")) {
                    if (line.Equals(versionTag)) currentBlock = UstBlock.Version;
                        ...
                        if (currentLines.Count != 0) {
                            ...
                            if (!currentNote.Lyric.Replace("R", string.Empty).Replace("r", string.Empty)
                                .Equals(string.Empty)) part.Notes.Add(currentNote);
                ...
                if (pbm != string.Empty) {
                    ...
                    for (int i = 0; i < m.Count() - 1; i++) {
                        pts[i].Shape = m[i] == "r" ? PitchPointShape.SINE_OUT :
                            ...
                            m[i] == "j" ? PitchPointShape.SINE_IN : PitchPointShape.SINE_IN_OUT;
                    }
                }
            }
